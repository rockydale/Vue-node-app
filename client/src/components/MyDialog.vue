      ...
      <div class="form">
        ...
        >
          ...
          <el-form-item class="text_right">
            <el-button @click="mydialog.show = false">取消</el-button>
...
export default {
  ...
  data() {
    return {
      format_type_list: ["提现", "充值", "优惠券", "充值礼券", "转账"],
      form_rules: {
        describe: [
  ...
  methods: {
    onSumbit(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          const url =
            this.mydialog.option == "add" ? "add" : `edit/${this.formData.id}`;
          this.$axios
